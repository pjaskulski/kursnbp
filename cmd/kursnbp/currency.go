...
func (c *NBPCurrency) GetCurrency(dFlag string, lFlag int, cFlag string) error {
	var err error

	if lFlag != 0 {
		c.result, err = getCurrencyLast(c.tableType, strconv.Itoa(lFlag), cFlag)
	} else if dFlag == "today" {
		c.result, err = getCurrencyToday(c.tableType, cFlag)
	} else if dFlag == "current" {
		c.result, err = getCurrencyCurrent(c.tableType, cFlag)
	} else if len(dFlag) == 10 {
		c.result, err = getCurrencyDay(c.tableType, dFlag, cFlag)
	} else if len(dFlag) == 21 {
		c.result, err = getCurrencyRange(c.tableType, dFlag, cFlag)
	}
